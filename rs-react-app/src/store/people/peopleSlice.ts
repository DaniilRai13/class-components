...
export const peopleSlice = createSlice({
  ...
  reducers: {
    toggleMarkedPeoples: (state, { payload }: { payload: IPeople }) => {
      const index = state.markedPeoples.findIndex((item) => item.id === payload.id)
      if (index !== -1) {
  ...
  extraReducers(builder) {
    builder.addMatcher(peopleApi.endpoints.getListPeoples.matchPending, (state) => {
      state.isLoading = true
    })
    builder.addMatcher(peopleApi.endpoints.getListPeoples.matchFulfilled, (state, { payload }) => {
